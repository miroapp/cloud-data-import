...
async function enrichBucketData(
	...
	const getBucketTagging = async () =>
		rateLimiter.throttle(() =>
			client
				.send(new GetBucketTaggingCommand({Bucket: bucket.Name}))
				.then((res) => res.TagSet)
				.catch(() => undefined),
		)

	const getBucketLocationConstraint = async () =>
		rateLimiter.throttle(() =>
			client
	...
	const [bucketPolicy, bucketVersioning, bucketEncryption, bucketTagging, bucketLocationConstraint] = await Promise.all(
		[
			getBucketPolicy(),
			getBucketVersioning(),
			getBucketEncryption(),
...
export async function getS3Resources(
	...
	const buckets = await getS3Buckets(credentials, rateLimiter)
