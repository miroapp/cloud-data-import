...
async function getCloudFrontDistributions(
	...
	const enrichedDistributions = await Promise.all(
		(response.DistributionList?.Items || []).map(async (distribution) => {
			const [distributionConfig, tags] = await Promise.all([
				rateLimiter.throttle(() =>
					client
						.send(new GetDistributionConfigCommand({Id: distribution.Id}))
						.then((res) => res.DistributionConfig)
						.catch(() => undefined),
				),
				rateLimiter.throttle(() =>
					client
						.send(new ListTagsForResourceCommand({Resource: distribution.ARN}))
						.then((res) => res.Tags)
						.catch(() => undefined),
				),
			])
